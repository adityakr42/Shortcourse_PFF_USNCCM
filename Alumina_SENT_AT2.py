###########################################################################################################################################################################################################################
#This FEniCS code implements the classical AT2 and AT1 phase-field model to solve the 'Surfing problem' to show that fracture propagation follows the Griffith principle.
# 
#
# Input: Set the material properties in lines 19-21. Then set the regularization length epsilon in line 25. 
#		 Set the value of notch length in line 33.
#
...
domain2 = CompiledSubDomain("x[1]<8*eps && x[0]<a+eps*10 && x[0]>a-eps*8", a=ac, eps=eps, h=h)
ir=0
while ir<3:
	d_markers = MeshFunction("bool", mesh, 2, False)
...
cracktip = CompiledSubDomain("x[1]<1e-4 && x[0]>a-eps*4 && x[0]<a+1e-4 ", a=ac, eps=eps)
...
phase_model=2;  #1 for AT1 model, 2 for AT2 model
...
sigma_external=1.2*sqrt(E*Gc/np.pi/ac)/((0.752+2.02*(ac/W)+0.37*(1-np.sin(np.pi*ac/2/W))**3)*(sqrt(2*W/np.pi/ac*np.tan(np.pi*ac/2/W)))/(np.cos(np.pi*ac/2/W)))
...
if phase_model==1:
	R_z = y*2*z*(psi11)*dx+ 3*Gc/8*(y*(-1)/eps + 2*eps*inner(grad(z),grad(y)))*dx + derivative(Wv,z,y)+ derivative(Wv2,z,y) # #linear model
else:
	R_z = y*2*z*(psi11)*dx+ Gc*(y*(z-1)/eps + eps*inner(grad(z),grad(y)))*dx + derivative(Wv2,z,y) #  #quadratic model
...
while t-stepsize < T:
	...
	z_x = evaluate_function(z, (ac+2*eps,0.0))
	if comm_rank==0:
		print(Fx)
		print(z_x)
		with open('Alumina_SENT_AT2.txt', 'a') as rfile: 
