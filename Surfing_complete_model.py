...
delta=1.14 #delta is calibrated by solving a separate boundary value problem for a particular eps and h
...
if phase_model==1:
	R_z = y*2*z*(psi11)*dx + y*(ce)*dx +3*Gc/8*(y*(-1)/eps + 2*eps*inner(grad(z),grad(y)))*dx + derivative(Wv,z,y) +  derivative(Wv2,z,y)  #linear model
else:
...
while t-stepsize < T:
	...
	surfenergy = 3/8*((1-z)/lch + lch*inner(grad(z),grad(z)))*dx
	...
	if comm_rank==0:
		print(Fx)
		with open('Surfing_ce.txt', 'a') as rfile:
			rfile.write("%s %s\n" % (str(t), str(Jintegral)))
		

	####Plot solution on incremental steps
	if step % printsteps==0:
		file_results = XDMFFile( "FullModel/Surfing_ce_" + str(step) + ".xdmf" )
